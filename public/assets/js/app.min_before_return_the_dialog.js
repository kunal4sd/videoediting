...
var WebVideoEditor = function(t) {
    ...
    this.init = function() {
        ...
        e.empty(), $.ajax({
            ...
            if (i.success) {
				$("#wve-list_" + t).text('');
                var n = _.template($("#listItemTemplate_" + t).html()), //from main_templates.html.php file id=listItemTemplate_input check line 222
                    s = _.template($("#listEmptyTemplate_" + t).html());
                i.data && i.data.length > 0 ? i.data.forEach(function(t, i) {
                    t.index = i, e.append(n(t)) /*rami*/
                }) : e.append(s())
            }
        })
        ...
        if (e.empty(), t.toggle(r.length > 0), 0 != r.length) {
            ...
            r.forEach(function(t, n) {
                t.index = n, t.imageUrl = a.options.baseUrl + a.options.requestHandler, t.imageUrl += "?action=frame_image&time=" + t.time[0] + "&itemId=" + t.id, e.append(i(t))
            })
        }
		...
		/*Rami When click on create movie
		alert(JSON.stringify(a.getProjectData()) );
		*/
        ...
        $.ajax({
			...
			data: {
				action: "render",
				data: JSON.stringify( a.getProjectData())
			},
			dataType: "json",
			cache: !1	
		}).done(function(t) {
...
function addMoreKeywords(){
  $('.keyword:last').clone(true).insertAfter('.keyword:last');
  $('.keyword:last').val('');
  return false;
}
