...
class DocumentCleaner extends AbstractModule implements ModuleInterface {
    ...
    private $searchNodes = [
        'combx', 'retweet', 'mediaarticlerelated', 'menucontainer', 
        ...
        'runaroundLeft', 'subscribe', 'vcard', 'articleheadings', 
        'author-dropdown', 'tools', 'socialtools', 'byline',
        'konafilter', 'KonaFilter', 'breadcrumbs', 'wp-caption-text', 'source',
        'legende', 'ajoutVideo', 'timestamp', 'js_replies', 'creative_commons', 'topics',
        'pagination', 'mtl', 'credit', 'toc_container', 'sharedaddy','storynote'
		//'popup', 'navbar',  'author','date', removed by rami yacoub
    ];
    ...
    public function run(Article $article) {
        ...
        $this->replace('em, strong, b, i, strike, del, ins', function($node) {    return !$node->find('img')->count();   });
        $this->replace('span[class~=dropcap], span[class~=drop_cap]');
        $this->remove('script, style');
        //$this->remove('header, footer, input, form, button, aside'); remove header,  form and aside  by rami yacoub
		$this->remove('footer, input,  button');
        $this->removeBadTags();
		...
		/*By rami yacoub - remove by style*/
		$this->remove("[style*='padding:10px 0 5px 0;'],[style*='color: rgb(255, 0, 0);']");
