...
class MetaExtractor extends AbstractModule implements ModuleInterface {
    ...
    private function cleanTitle($title) {
		...
		/*By rami yacoub remove title array*/
		$remove_from_title = array(
		'صحيفة مكة ::','اليوم :','الوطن أون لاين :::','جريدة النهار :: '
		);
        if (isset($openGraph['site_name']) && ($openGraph['site_name'] != $title ) ) { //Rami yacoub
		...
		$title = trim(str_replace($remove_from_title,'',$title));
    ...
    private function getTitle() {
        ...
        $nodes = $this->getNodesByLowercasePropertyValue($this->article()->getDoc(), 'meta', 'name', 'og:title');
        if ($nodes->count()) {
            return $this->cleanTitle($nodes->first()->attr('content'));
        }
		
        $nodes = $this->getNodesByLowercasePropertyValue($this->article()->getDoc(), 'h2', 'class', 'articletitle red');
        if ($nodes->count()) {
            return $this->cleanTitle($nodes->first()->text());
        }		
	/* By Rami yacoub*/
        $nodes = $this->article()->getDoc()->find('#lblHeading');
        if ($nodes->count()) {
    ...
    private function getMetaLanguage() {
		...
		$pos = strpos($this->article()->getRawHtml(), 'ي');
		if($pos){ // edit by rami yacoub
			return 'ar';
		}
