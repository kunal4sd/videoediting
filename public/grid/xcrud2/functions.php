...
function ins_encrypt_pass($postdata, $xcrud){
        $db = Xcrud_db::get_instance();
        $query = 'SELECT password("'.$postdata->get('user.password').'") as pass';
        $db->query($query);
        $result = $db->result();
		$postdata->set('password', $result[0]['pass']);	
}

function encrypt_pass($postdata, $primary, $xcrud){
        $db = Xcrud_db::get_instance();
        $query = 'SELECT password("'.$postdata->get('user.password').'") as pass';
        $db->query($query);
        $result = $db->result();
