        ...
        a.Chart.prototype.getTable = function(a) {
            ...
            f += function(a, d, c) {
                ...
                if (d) {
                    ...
                    for (c = d.length; e < c; ++e) {
						var key = "name"+e;
						if (typeof sumArr[key]  == 'undefined') {
							sumArr[key] = d[e];
						} 
            ...
            c(a, function(a) {
                ...
                for (var d = 0; d < e; d++) {
					if(d){
						if(a[d]){
							var links2 = "\x3ca href='json/dashboard/show_emp_articles.php?emp_name="+sumArr["name"+d]+"\x26cur_date="+cur_date+"\x26act_name="+cuAction+"' class='popup' \x3e"+a[d]+"\x3c/a\x3e"						
						} else {
							var links2 = '-';
						}
						f += q(d ? "td" : "th", null,  "" , links2);
					} else {
						f += q(d ? "td" : "th", null, 'scope\x3d"row"', a[d]);
						var cur_date = a[d];
					}
					if(d >= 1){
						if(Number.isInteger(a[d]) ) {
							...
							sumArr[key] += Number(a[d])
						}
					}
				}
                f += "\x3c/tr\x3e"
				console.log("the length of arr is: " + sumArr.length); // 3
			...
			for (var d = 0; d < e; d++) {
				if(d == 0){
					...
					var links = "\x3ca href='json/dashboard/show_emp_articles.php?emp_name="+sumArr["name"+d]+"\x26fdate="+sfdate+"\x26tdate="+stdate+"\x26act_name="+cuAction+"' class='popup' \x3e"+sumArr["sum"+d]+"\x3c/a\x3e"
					f += q("td" , null,  "" ,links );
				}
			}			
