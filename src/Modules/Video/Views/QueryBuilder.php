...
class QueryBuilder extends AbstractModule
{
    public function __invoke(Request $request, Response $response, $args)
    {
        ...
        $keywords               = [];
        try {
            foreach ($this->entity_user->get_all_users() as $userAr){
                $users[] = $userAr->build_to_array();
            }
            foreach ($this->entity_keyword->get_all_keywords() as $keywordAr){
                $keywords[] = $keywordAr->build_to_array();
            }
        ...
        return $this->view->render($response, 'query-builder/index.twig', [
            'page_title'    => 'Search Query Builder',
            'page_name'     => 'query-builder',
