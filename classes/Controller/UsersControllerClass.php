    ...
    public function deleteUserPage( $userId = 0 )
    {
        ...
        if( $submited ){
            $redirectUrl = $currentUser['role'] == 'admin'
                ? $this->config['base_url'] . '?action=users'
                : $this->config['base_url'];
            self::redirectTo( $redirectUrl );
        }

        $output['content'] = '<form action="' . $this->config['base_url'] . '?action=delete_user&user_id=' . $userId . '" method="post">';

        if( $currentUser['id'] == $userId ){
            $output['content'] .= '<p>Are you sure you want to delete your account?</p>';
        } else {
            $output['content'] .= '<p>Are you sure you want to remove user?</p>';
        }

        $output['content'] .= '<div>';
        $output['content'] .= ' <button type="submit" class="btn btn-primary" name="accept" value="1">Yes</button>';
        $output['content'] .= ' <button type="submit" class="btn btn-secondary" name="cancel" value="1">No</button>';
        $output['content'] .= '</div>';
        $output['content'] .= '</form>';
